...
def measure_restart_intervals(jpeg_filepath: Path):
    ...
    print(f"\n--- ファイル '{jpeg_filepath.name}' のリスタートマーカー間隔を測定中 ---")
...
if __name__ == "__main__":
    print("=== JPEGリスタートマーカー解析ツール ===")
    input_path = input("解析したいJPEGファイルのパスを入力してください: ").strip('"')

    if not input_path:
        print("エラー: パスが入力されていません。終了します。")
        sys.exit(1)

    target_jpeg_path = Path(input_path)
    measure_restart_intervals(target_jpeg_path)
